...
+ (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    for (id<XCPayFactoryProtocol> factory in [self payFactorys]) {
        ...
        if ([pay respondsToSelector:@selector(application:didFinishLaunchingWithOptions:)]) {
...
+ (BOOL)application:(UIApplication *)application continueUserActivity:(NSUserActivity *)userActivity restorationHandler:(void (^)(NSArray<id<UIUserActivityRestoring>> *))restorationHandler
{
    ...
    for (id<XCPayFactoryProtocol> factory in [self payFactorys]) {
        id<XCPayProtocol> pay = factory.pay;
        if ([pay respondsToSelector:@selector(application:continueUserActivity:restorationHandler:)]) {
            BOOL result = [pay application:application continueUserActivity:userActivity restorationHandler:restorationHandler];
            if (result) {
                isSuccess = YES;
            }
        }
